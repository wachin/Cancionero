...
from PyQt6.QtCore import Qt, QItemSelectionModel, QLocale, QLibraryInfo, QTranslator
from PyQt6.QtGui import QIcon
from PyQt6.QtWidgets import (
    QApplication, QWidget, QVBoxLayout, QHBoxLayout, QListWidget, QListWidgetItem,
    QPushButton, QFileDialog, QLabel, QLineEdit, QMessageBox, QComboBox, QFrame
)
...
def soffice_convert(input_path: Path, to_ext: str, out_dir: Path) -> Path:
    ...
    if not produced.exists():
        # LibreOffice a veces produce sufijos raros; buscar por mismo stem y ext
...
def merge_odt(files: list[Path], out_path: Path) -> None:
    # Ruta robusta: ODT→DOCX, unir, y si la salida pedida es .odt, reconvertir
    try:
        from docx import Document
        from docxcompose.composer import Composer
    except Exception as e:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for f in files:
            if f.suffix.lower() != '.odt':
                raise ValueError(f"Archivo no ODT en modo ODT: {f}")
...
class FileListWidget(QListWidget):
    """Lista que acepta archivos por arrastrar y soltar (D&D)."""
        ...
        for url in event.mimeData().urls():
            if url.isLocalFile():
                ...
                if p.exists() and p.is_file():
                    paths.append(p)
        if hasattr(self.parent(), 'add_files'):
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        btn_validate = QPushButton('Validar tipos')
        ...
        mode_row.addWidget(QLabel('Modo de unión:'))
        self.mode_combo = QComboBox()
        self.mode_combo.addItems(['TXT', 'DOCX', 'ODT'])
        ...
        out_row1.addWidget(QLabel('Nombre de salida:'))
        self.output_name = QLineEdit('merged.txt')
    ...
    def add_files(self, paths: list[Path]):
        ...
        for p in paths:
            ...
            if p.suffix.lower() not in exts:
                # ignora tipos no válidos para el modo actual
                continue
    ...
    def on_add(self):
        ...
        if mode == 'TXT':
            ...
            flt = 'OpenDocument Text (*.odt)'
        files, _ = QFileDialog.getOpenFileNames(self, 'Seleccionar archivos', str(Path.home()), flt)
    ...
    def on_sort(self):
        ...
        paired.sort(key=lambda x: (x[0].name.lower(), str(x[0]).lower()))
    ...
    def on_validate_types(self):
        mode = self.mode_combo.currentText()
        exts = {'TXT': '.txt', 'DOCX': '.docx', 'ODT': '.odt'}[mode]
        bad_rows = [i for i, p in enumerate(self.files) if p.suffix.lower() != exts]
        if bad_rows:
            QMessageBox.warning(self, 'Tipos no válidos', f"Se eliminarán {len(bad_rows)} elemento(s) que no son {exts}.")
            for i in reversed(bad_rows):
    ...
    def on_mode_changed(self, mode: str):
        # Cambiar extensión sugerida
        exts = {'TXT': '.txt', 'DOCX': '.docx', 'ODT': '.odt'}
        name = self.output_name.text().strip() or 'merged'
        base = Path(name).stem
        self.output_name.setText(base + exts[mode])
        # Filtro rápido: eliminar elementos que no coincidan (opcional). Aquí no eliminamos, solo advertimos.
    ...
    def on_join(self):
        ...
        mode = self.mode_combo.currentText()
        name = self.output_name.text().strip() or 'merged'
        desired_ext = {'TXT': '.txt', 'DOCX': '.docx', 'ODT': '.odt'}[mode]
        ...
        # Validar tipos
        valid_suffix = desired_ext
        wrong = [p for p in self.files if p.suffix.lower() != valid_suffix]
        if wrong:
            QMessageBox.warning(self, 'Tipos mezclados', f"Estos archivos no coinciden con el modo {mode}:\n" + "\n".join(str(p) for p in wrong))
            return
...
def install_translators(app: QApplication):
    ...
    try:
        ...
        # Qt6 suele usar estos nombres de catálogo
        ...
        # Nota: Las cadenas propias de esta app están en español por defecto.
    except Exception:
        pass
